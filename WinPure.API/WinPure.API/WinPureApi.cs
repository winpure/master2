...
namespace WinPure.API
{
    public class WinPureApi
    {
        ...
        public WinPureApi()
        {
            var resolver = new WinPureApiDependencyResolver();
            _serviceProvider = resolver.ServiceProvider;
            _licenseService = _serviceProvider.GetService(typeof(ILicenseService)) as ILicenseService;
            _licenseService.Initiate(Common.Enums.ProgramType.Api);
        ...
        public LicenseState Register(string licenseKey)
        {
            return _licenseService.Register(licenseKey);
        }
        ...
        /// <summary>
        /// In-memory United Kingdom address verification. 
        /// </summary>
        /// <param name="data">Table with source data</param>
        /// <param name="settings">Verification settings</param>
        /// <param name="cancellationToken">Cancellation token for cancelling verification</param>
        /// <returns>Address verification report</returns>
        public async Task<AddressVerificationReport> VerifyUkAddressesAsync(DataTable data, UkAddressVerificationSettings settings, CancellationToken cancellationToken)
        {
            var addressVerificationService = _serviceProvider.GetService(typeof(IUkAddressVerificationService)) as IUkAddressVerificationService;
            var report = await addressVerificationService.VerifyAddresses(data, settings, cancellationToken, RaiseOnProgressUpdate, RecordCountForAddressVerification());
            return report;
        }
        ...
        /// <returns>Address verification report</returns>
        public AddressVerificationReport VerifyUkAddresses(DataTable data, UkAddressVerificationSettings settings, CancellationToken cancellationToken)
        {
            return VerifyUkAddressesAsync(data, settings, cancellationToken).Result;
        }

        /// <summary>
        /// USA address verification in database. 
        /// </summary>
        /// <param name="dbPath">Path to SQLite database with source data</param>
        /// <param name="tableName">Name of table in the database</param>
        /// <param name="settings">Verification settings</param>
        /// <param name="cancellationToken">Cancellation token to cancel the verification</param>
        /// <returns></returns>
        public AddressVerificationReport VerifyUsAddressesLocal(string dbPath, string tableName, UsAddressVerificationSettings settings, CancellationToken cancellationToken)
        {
            var addressVerificationService = _serviceProvider.GetService(typeof(IUsAddressVerificationInDatabaseService)) as IUsAddressVerificationInDatabaseService;
            var report = addressVerificationService?.VerifyAddresses(dbPath, tableName, settings, cancellationToken, RaiseOnProgressUpdate, RecordCountForAddressVerification());
            return report;
        }
        ...
        {
            var cleansingService = _serviceProvider.GetService(typeof(ICleansingService)) as ICleansingService;
            return cleansingService?.CalculateStatistic(data, cancellationToken);
        }
        ...
        {
            var dataNormalizationService = _serviceProvider.GetService(typeof(IDataNormalizationService)) as IDataNormalizationService;
            return dataNormalizationService?.MergeMatchResult(matchResult, mergeSettings, valueSeparator, (s, i) => RaiseOnProgressUpdate(s, i, 0, 0));
        }
        ...
        /// <summary>
        /// Update the result acoording to specified mergeSettings. 
        /// </summary>
        /// <param name="matchResult">Datatable with matching result. It should be same table that was returned by MatchData function.</param>
        ...
        {
            var dataNormalizationService = _serviceProvider.GetService(typeof(IDataNormalizationService)) as IDataNormalizationService;
            dataNormalizationService?.UpdateMatchResult(matchResult, updateSettings, (s, i) => RaiseOnProgressUpdate(s, i, 0, 0));
        }
        ...
        private void RaiseOnProgressUpdate(string description, int progress, int currentStep = 0, int totalSteps = 10)
        {
            OnProgress?.Invoke(description, progress, currentStep, totalSteps);
        }
    }
}