...
internal static partial class WinPureCleansingDependencyExtension
{
    ...
    {
        private static List<string> TextCleanSettingsFields => ["RM_Others", "RE_Expression", "RE_Replace", "RE_Copy", "CV_EmptyToDefault", "ST_ProperCaseSettings", "AI_Type", "AI_Label"];
        ...
        /// <param name="destinationPath">Destination folder</param>
        /// <param name="fileName">File name (without extension).</param>
        public void ExportProperCaseConfiguration(ProperCaseConfiguration settings, string fileName)
        {
            if (settings == null)
            {
                throw new WinPureArgumentException(Resources.EXCEPTION_SETTINGSNOTNULL);
            ...
            {
                _logger?.Error("Cannot export proper case configuration to file", exception);
                throw new WinPureCannotProcessFile(Resources.EXCEPTION_EXPORT_SETTING_ERROR, exception);
            }
        }
        ...
        /// <returns></returns>
        public ProperCaseConfiguration ImportProperCaseConfiguration(string destinationFile)
        {
            try
            {
                var content = FileHelper.ReadFile(destinationFile);
                var settings = JsonConvert.DeserializeObject<ProperCaseConfiguration>(content);
                return settings;
            }
            catch (Exception exception)
            {
        ...
        {
            var selectCleanSettingsQuery = SqLiteHelper.GetCleanSettingsQuery(tableName);
            var cleanSettings =  SqLiteHelper.ExecuteQuery(selectCleanSettingsQuery, _connectionManager.Connection, CommandBehavior.Default, tableName);

            cleanSettings = SqLiteHelper.ExecuteQuery(selectCleanSettingsQuery, _connectionManager.Connection, CommandBehavior.Default, tableName);

            return cleanSettings;
        }

        public DataTable ConvertCleanSettingsToDataTable(string tableName, WinPureCleanSettings cleanSettings)
        {
            var cleanSettingsTable = SqLiteHelper.ExecuteQuery(SqLiteHelper.GetCleanSettingsQuery(tableName), _connectionManager.Connection, CommandBehavior.SchemaOnly, tableName);
            ...
            foreach (var converterSetting in cleanSettings.CaseConverterSettings)
            {
                ...
                row["ST_ProperCaseSettings"] = JsonConvert.SerializeObject(converterSetting.ProperCaseSettings);
            }
            ...
            foreach (var textCleanerSetting in cleanSettings.TextCleanerSettings)
            {
                ...
                row["RE_Expression"] = JsonConvert.SerializeObject(textCleanerSetting.RegexExpression);
            }
            ...
            foreach (var addressColumn in cleanSettings.AddressAndGenderSplitSettings.AddressColumns)
            {
                var row = GetColumnRow(addressColumn);
                row["SP_Address"] = 1;
            }

            foreach (var countryColumn in cleanSettings.AddressAndGenderSplitSettings.CountryColumns)
            {
                var row = GetColumnRow(countryColumn);
                row["SP_Country"] = 1;
            }

            foreach (var genderColumn in cleanSettings.AddressAndGenderSplitSettings.GenderColumns)
            {
                var row = GetColumnRow(genderColumn);
                row["SP_Gender"] = 1;
            }
        ...
        public WinPureCleanSettings GetWinPureCleanSettings(string tableName)
        {
            var cleanSettingsDataTable = GetDataTableCleanSettings(tableName);

            var cleanSettings = new WinPureCleanSettings();
            int shiftIndex = 0;

            var genderColumnList = new List<string>();
            var countryColumnList = new List<string>();
            var addressColumnList = new List<string>();
            var cityColumnList = new List<string>();
            var postcodeColumnList = new List<string>();
            var regionColumnList = new List<string>();

            foreach (DataRow rw in cleanSettingsDataTable.Rows)
            {
                if (rw["ST_UpperCase"].ToString() == "1" || rw["ST_LowerCase"].ToString() == "1" || rw["ST_ProperCase"].ToString() == "1" || !string.IsNullOrWhiteSpace(rw["ST_ProperCaseSettings"].ToString()))
                {
                    var ccSett = new CaseConverterSetting
                    {
                        ...
                        ProperCaseSettings = string.IsNullOrEmpty(rw["ST_ProperCaseSettings"].ToString())
                    ...
                    || rw["RE_Expression"].ToString().Trim() != "")
                {
                    var cleanSett = new TextCleanerSetting
                    {
                        ...
                        RegexExpression = string.IsNullOrEmpty(rw["RE_Expression"].ToString()) ? new() : JsonConvert.DeserializeObject<List<RegexConfigurationSetting>>(rw["RE_Expression"].ToString().Trim()),
                    };
                ...
                if (rw["SP_EmailAddress"].ToString() == "1" || rw["SP_Email"].ToString() == "1" || rw["SP_Telephones"].ToString() == "1"
                    || rw["SP_DateTime"].ToString() == "1" || rw["SP_Words"].ToString() == "1"
                    || rw["RE_Copy"].ToString().Trim() != "")
                {
                ...
                if (rw["SP_Address"].ToString() == "1")
                {
                    addressColumnList.Add(rw["COLUMN_NAME"].ToString());
                }
                if (rw["SP_Country"].ToString() == "1")
                {
                    countryColumnList.Add(rw["COLUMN_NAME"].ToString());
                }
                if (rw["SP_Gender"].ToString() == "1")
                {
                    genderColumnList.Add(rw["COLUMN_NAME"].ToString());
                }
                if (rw["SP_City"].ToString() == "1")
                {
                    cityColumnList.Add(rw["COLUMN_NAME"].ToString());
                }
                if (rw["SP_Postcode"].ToString() == "1")
                {
                    postcodeColumnList.Add(rw["COLUMN_NAME"].ToString());
                }
                if (rw["SP_Region"].ToString() == "1")
                {
                    regionColumnList.Add(rw["COLUMN_NAME"].ToString());
                }
            }

            if (genderColumnList.Any() || addressColumnList.Any() || countryColumnList.Any())
            {
                cleanSettings.AddressAndGenderSplitSettings.AddressColumns = addressColumnList;
                cleanSettings.AddressAndGenderSplitSettings.GenderColumns = genderColumnList;
                cleanSettings.AddressAndGenderSplitSettings.CountryColumns = countryColumnList;
                cleanSettings.AddressAndGenderSplitSettings.CityColumns = cityColumnList;
                cleanSettings.AddressAndGenderSplitSettings.PostcodeColumns = postcodeColumnList;
                cleanSettings.AddressAndGenderSplitSettings.RegionColumns = regionColumnList;
            }

            var tableWM = SqLiteHelper.ExecuteQuery(SqLiteHelper.GetSelectQuery(NameHelper.GetWordManagerTable(tableName)), _connectionManager.Connection);
            ...
            if (cleanSettings.ColumnMergeSettings.Count == 1)
            {
                cleanSettings.ColumnMergeSettings.Clear(); // one column we should not merge
            }
        ...
        public void SaveCleanSettings(string tableName, string columnName, string fieldName, object value)
        {
            _connectionManager.CheckConnectionState();

            var textValue = value == null ? String.Empty : value.ToString()?.Replace("'", "''");

            var sql = TextCleanSettingsFields.Contains(fieldName)
                ? $"UPDATE [{NameHelper.GetCleanSettingsTable(tableName)}] SET {fieldName}='{textValue}' WHERE COLUMN_NAME='{columnName}'"
                : $"UPDATE [{NameHelper.GetCleanSettingsTable(tableName)}] SET {fieldName}={value} WHERE COLUMN_NAME='{columnName}'";

            SqLiteHelper.ExecuteNonQuery(sql, _connectionManager.Connection);
        }

        public DataTable GetColumnUniqueValues(string tableName, string columnName, bool refreshData, TextCleanerSetting cleanerSett = null, CaseConverterInternalSetting caseSett = null)
        {
            _connectionManager.CheckConnectionState();

            var sql = SqLiteHelper.GetSelectQuery(NameHelper.GetColumnValuesTable(tableName), "[COLUMN_VALUE], [VALUE_COUNT]", $"[COLUMN_NAME]='{columnName}'");
            var res = SqLiteHelper.ExecuteQuery(sql, _connectionManager.Connection);
            if (res.Rows.Count == 0 && refreshData)
            {
                UpdateUniqueValuesForColumn(tableName, columnName);
            ...
            if (cleanerSett != null)
            {
                ...
                if (cleanerSett.RegexExpression != null && cleanerSett.RegexExpression.Count > 0)
                {
                    foreach (var regexSetting in cleanerSett.RegexExpression)
                    {
                        if (string.IsNullOrWhiteSpace(regexSetting?.Expression))
                        {
                            continue;
                        }

                        var invertMatch = string.Equals(regexSetting.Replacement, "NOT", StringComparison.OrdinalIgnoreCase);

                        qry = invertMatch
                            ? qry.Where(x => !Regex.IsMatch(x.Field<string>("COLUMN_VALUE"), regexSetting.Expression))
                            : qry.Where(x => Regex.IsMatch(x.Field<string>("COLUMN_VALUE"), regexSetting.Expression));
                    }
                }
            ...
            if (caseSett != null)
            {
                ...
                if (caseSett.ToMixedCase)
                {
                    var textInfo = new CultureInfo("en-US", false).TextInfo;
                    qry = qry.Where(x => x.Field<string>("COLUMN_VALUE").ToLower() != x.Field<string>("COLUMN_VALUE") 
                                         && x.Field<string>("COLUMN_VALUE").ToUpper() != x.Field<string>("COLUMN_VALUE")
                                         && !x.Field<string>("COLUMN_VALUE").Equals(textInfo.ToTitleCase(textInfo.ToLower(x.Field<string>("COLUMN_VALUE")))));
                }
                res = (qry.Any()) ? qry.CopyToDataTable() : res.Clone();
            }
            return res;
        ...
        public List<WordManagerSetting> GetWordManagerSettings(string tableName, string columnName)
        {
            _connectionManager.CheckConnectionState();
            var sql = SqLiteHelper.GetSelectQuery(NameHelper.GetWordManagerTable(tableName), "", $"[TABLE_NAME]='{tableName}' AND [COLUMN_NAME]='{columnName}'");
            ...
            return dat.AsEnumerable().Select(x => new WordManagerSetting
            {
                ...
                ReplaceType = (WordManagerReplaceType)x.Field<long>("ReplaceOption"),
                ReplaceValue = x.Field<string>("ToReplace"),
                SearchValue = x.Field<string>("COLUMN_VALUE")
            }).ToList();
        }

        public void SaveWordManagerSettings(string tableName, List<WordManagerSetting> wmWordManagerSettings, string columnName)
        {
            _connectionManager.CheckConnectionState();
            ...
            if (wmWordManagerSettings.Any())
            {
                ...
                {
                    sql = SqLiteHelper.GetDeleteQuery(NameHelper.GetWordManagerTable(tableName), $"[TABLE_NAME]='{tableName}' AND [COLUMN_NAME]='{col}'");
                    SqLiteHelper.ExecuteNonQuery(sql, _connectionManager.Connection);
                }
                ...
                foreach (var sett in wmWordManagerSettings)
                {
                    ...
                    sql = $"INSERT INTO [{NameHelper.GetWordManagerTable(tableName)}] (TABLE_NAME, COLUMN_NAME, COLUMN_VALUE, ToDelete, ToReplace, ReplaceOption) VALUES ('{tableName}','{sett.ColumnName}','{sett.SearchValue.Replace("'", "''")}',{toDelete},'{sett.ReplaceValue.Replace("'", "''")}',{opt})";
                    SqLiteHelper.ExecuteNonQuery(sql, _connectionManager.Connection);
                }
            }
            else
            {
                var sql = SqLiteHelper.GetDeleteQuery(NameHelper.GetWordManagerTable(tableName), $"[TABLE_NAME]='{tableName}' AND [COLUMN_NAME]='{columnName}'");
                SqLiteHelper.ExecuteNonQuery(sql, _connectionManager.Connection);
            }
        }

        public List<ReportResultData> GetWordManagerColumnValues(string tableName, string columnName)
        {
            _connectionManager.CheckConnectionState();
            var sql = SqLiteHelper.GetSelectQuery(NameHelper.GetColumnValuesTable(tableName), "[COLUMN_VALUE] as ColVal, [VALUE_COUNT] as ColValCnt", $"[COLUMN_NAME]='{columnName}'");
        ...
        private void CreateCleanSettingsTable(string tableName)
        {
            ...
            string sql = "create table [" + tableName + "] (COLUMN_NAME TEXT PRIMARY KEY, " +
                              ...
                              "ST_ProperCaseSettings TEXT DEFAULT ''," +
                              ...
                              "SP_Address INTEGER DEFAULT 0," +
                              "SP_City INTEGER DEFAULT 0," +
                              "SP_Region INTEGER DEFAULT 0," +
                              "SP_Postcode INTEGER DEFAULT 0," +
                              "SP_Country INTEGER DEFAULT 0," +
                              "SP_Gender INTEGER DEFAULT 0," +
                              ...
                              "RG_Latitude INTEGER DEFAULT 0," +
                              "RG_Longitude INTEGER DEFAULT 0," +
                              "AI_Type TEXT DEFAULT ''," +
                              "AI_Label TEXT DEFAULT ''," +
                              "AI_Include INTEGER DEFAULT 1," +
                              "AI_Ignore INTEGER DEFAULT 0," +
